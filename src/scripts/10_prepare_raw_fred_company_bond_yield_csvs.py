...
FRED_SERIES_TO_FETCH = {
    "BAMLH0A0HYM2": "fred_BAMLH0A0HYM2_raw_vintage.csv",
    "BAMLC0A4CBBB": "fred_BAMLC0A4CBBB_raw_vintage.csv",
}
...
if __name__ == "__main__":
    ...
    logger.info("Starting raw FRED yield data fetching process...")

    for series_id, filename in FRED_SERIES_TO_FETCH.items():
        ...
        try:
            ...
            if data_df is not None and not data_df.empty:
                ...
                logger.info(
                    f"Successfully fetched and saved {len(data_df)} records "
                    f"for {series_id} to {output_path}"
                )
    ...
    logger.info("Raw FRED yield data fetching script finished.")
